...
// 모든 주요 연산(삽입, 삭제, 검색)에 O(log n)의 시간 복잡도가 걸린다.
// std::unordered_set은 해시 테이블을 기반으로 하며, 평균적으로 O(1)의 시간 복잡도를 제공한다. 
// 그러나 최악의 경우, 모든 요소가 하나의 버킷에 모일 때 O(n)이 될 수 있다.

// 성능 차이의 주된 이유는 다음과 같다:
// - set은 트리 기반 구조로서 정렬된 상태를 유지하며, 원소 검색이 로그 시간 복잡도를 갖는다.
// - unordered_set은 해시 테이블을 사용하여 평균적으로 더 빠른 접근 시간을 제공하지만, 
...
int main() {
	...
	cout << "1. Insertion into set: " << durationSetInsert.count() << " ms" << endl;

	// unordered set 삽입 성능 측정
	start = high_resolution_clock::now();
	for (int i = 0; i < NUM_ELEMENTS; i++)
		myUnorderedSet.insert(i);
	end = high_resolution_clock::now();
	auto durationUnorderedSetInsert = duration_cast<milliseconds>(end - start);
	cout << "2. Insertion into unordered_set: " << durationUnorderedSetInsert.count() << " ms" << endl;
	...
	cout << "3. Search in set: " << durationSetSearch.count() << " ms" << endl;

	// unorderd set 검색 성능 측정
	start = high_resolution_clock::now();
	for (int i = 0; i < NUM_ELEMENTS; i++) {
	...
	cout << "4. Search in unorderd_set: " << durationUnorderedSetSearch.count() << " ms" << endl;
